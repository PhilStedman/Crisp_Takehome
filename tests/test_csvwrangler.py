...
def test_samplecsv():
    ...
    expected = [
        csvwrangler.Order(
            1000, datetime.datetime(2018, 1, 1), "P-10001", "Arugola", 5250.50, "kg"
        ),
        csvwrangler.Order(
            1001,
...
def test_samplecsv_withcfg():
    file = Path(__file__).parent / "testcsvs/sample.csv"
    cfgFile = Path(__file__).parent / "testcfgs/config1.ini"

    orders = csvwrangler.wrangle(file, cfgFile)

    expected = [
        csvwrangler.Order(
            1000, datetime.datetime(2018, 1, 1), "P-10001", "Arugola", 5250, "lbs"
        ),
        csvwrangler.Order(
            1001,
            datetime.datetime(2017, 12, 12),
...
def test_errorcsv():
    ...
    expected = [
        csvwrangler.Order(
            1000, datetime.datetime(2020, 6, 27), "P-9001", "Chocolate", 5.25, "kg"
        ),
        csvwrangler.Order(
            1001, datetime.datetime(2019, 12, 12), "P-10002", "Bagel", 15, "kg"
        ),
    ]
